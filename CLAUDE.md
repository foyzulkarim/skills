...
- **plan-qa** (post-implementation QA gate, independent of review) — QA planning. The developer chooses whether to run it before, alongside, or after `/review`. Three entry modes: pipeline (from ARCH), general (from a PR/branch diff), and bug (`/plan-qa bug #123` or `bug "<description>"` — the repro case asserting the fixed behavior plus regression smoke). Interviews the developer (walk the artifacts → mine the developer → confirm) to turn the specs and the diff into an executable QA specification: cases with tagged steps (`[bash]`/`[browser]`), `Guard:`-codified project traps, a Coverage Map over every changed file, logical identities (credentials come from the run's env file, never the plan), preconditions (P0 = automated suite), parallelism decided at plan time as **lanes** (collision rules over shared identities/state, lane 1 pinned for operator handoffs and log correlation), and named operator handoffs. Every Expected line is falsifiable — `[assert]` (machine-verifiable), `[judge]` with an explicit criterion, or `[judge-visual]` judged from a captured screenshot. Plans are environment-portable: relative URLs only, `[bash local-only]` tags for process-local steps. Outputs `/specs/qa/QA-<N>-<slug>.md`. Declaration skill: nothing project-specific is baked in; scenario types are open-ended (browser + shell today; storybook, performance, accessibility later). Skip when the change has no running surface worth driving.
- **execute-qa** (post-implementation QA gate, independent of review) — Executes a QA specification as written against a chosen environment (`--env <name>` loads a gitignored `.env.qa.<name>`; `--base <url>` overrides; environment failures fail hard at preconditions). Preconditions first (red P0 = no run), cases in order with their tagged drivers and guards, verbatim operator handoffs. `[browser]` steps run through the bundled `qa-browser.mjs` daemon (persistent headed Playwright session, one JSON line + exit code per command, per-lane isolated contexts via `--ctx`, auto-screenshot on failure); multi-lane plans run one subagent per lane and merge into the results. `[assert]` lines verify mechanically; `[judge]`/`[judge-visual]` lines are judged only against the plan's written criterion, with evidence quoted and ambiguity escalating to PARTIAL — never a guessed PASS. Writes verdicts (PASS / PASS (judged) / FAIL / PARTIAL / SKIPPED) and findings to `/specs/qa/QA-RESULTS-<N>-<slug>.md`, appending one section per run; never modifies the plan.
