---
name: plan-architecture-v2
description: "Design the system or feature architecture AND map the change footprint — Phase 2 of the 5-phase pipeline (v2: improved context gathering with -m 3 match cap, keyword calibration thresholds, and anti-speculative-read rules). Use this skill when the developer is ready to make solution-space decisions: high-level structure, tech choices, data models, API contracts, module boundaries, patterns, AND a concrete walk through the codebase identifying which files/modules get created, modified, or impacted. Reads `/specs/requirements/REQ-<slug>.md` when present, or runs from a brief. Produces `/specs/architecture/ARCH-<slug>.md`."
model: inherit
