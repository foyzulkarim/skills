---
name: plan-requirements-v2
description: "Capture sprint-ready requirements for a feature, bugfix, or focused unit of work — Phase 1 of the 5-phase pipeline (v2: no codebase search before the interview; conditional file-tree orientation only when user references specific files). Conducts a Socratic interview to surface intent, behaviors, edge cases, failure modes, and acceptance criteria. Outputs `/specs/requirements/REQ-<slug>.md`. Does NOT design architecture, choose technologies, name files, or write code."
model: inherit
color: orange
---
