...
Post-implementation QA planning — independent of `/review` (the developer chooses whether to run them sequentially or in parallel, and in what order). Three entry modes: pipeline (from ARCH), general (from a PR/branch diff), and bug (`/plan-qa bug #123` — the repro case asserting the fixed behavior plus regression smoke). Interviews the developer to turn the specs and the diff into an executable QA specification: cases with tagged steps (`[bash]`/`[browser]`), project traps codified as `Guard:`s on the exact steps that need them, a Coverage Map over every changed file, logical identities with credentials from the run's env file, preconditions, plan-time parallelism as lanes (lane 1 pinned for operator handoffs and log correlation), and named operator handoffs for the few actions an agent genuinely cannot do. Every Expected line is falsifiable — `[assert]` (machine-verifiable), `[judge]` with an explicit pass/fail criterion fixed at plan time, or `[judge-visual]` judged from a captured screenshot. Plans are environment-portable: relative URLs only, `[bash local-only]` tags for process-local steps. Produces `/specs/qa/QA-<N>-<slug>.md`. Skip this gate when the change has no running surface worth driving.


### /execute-qa (QA gate)

Runs after `/plan-qa` produces the specification, against a chosen environment (`--env <name>` loads a gitignored `.env.qa.<name>`; `--base <url>` overrides; missing env files or credentials and unreachable bases fail hard at preconditions). Preconditions run first (a red automated suite means the run does not begin); cases execute in order with their tagged drivers and guards; operator handoffs print verbatim and wait. `[browser]` steps run through the bundled `qa-browser.mjs` daemon — a persistent headed Playwright session with one JSON line + exit code per command; multi-lane plans run one subagent per lane with isolated contexts. `[assert]` lines verify mechanically; `[judge]`/`[judge-visual]` lines are judged only against the plan's written criterion, with the observed evidence quoted next to the verdict and ambiguity escalating to PARTIAL — never a guessed pass. Appends one run section of verdicts and findings to `/specs/qa/QA-RESULTS-<N>-<slug>.md`; never modifies the plan.
